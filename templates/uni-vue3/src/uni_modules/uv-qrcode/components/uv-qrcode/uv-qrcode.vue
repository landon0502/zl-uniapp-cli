<template>
	...
	>
		...
		<!-- 加载效果 -->
		<view class="uqrcode-makeing" v-if="loading === undefined || !loading ? makeing : loading">
			<slot name="loading">
				<image
		...
		<view class="uqrcode-h5-save" v-if="isH5Save">
			<slot name="h5save" :tempFilePath="tempFilePath">
				...
				<text class="uqrcode-h5-save-text"
					>{{ h5SaveIsDownload ? '若保存失败，' : '' }}请长按二维码进行保存</text
				>
			</slot>
...
export default {
	...
	mounted() {
		...
		if (this.canvasType == '2d') {
			// #ifndef MP-WEIXIN
			this.templateOptions.canvasTransform = `scale(${this.templateOptions.size / this.templateOptions.canvasWidth}, ${
		...
		toTempFilePath(callback = {}) {
			...
			if (this.canvasType === '2d') {
				...
				try {
					let dataURL = null
					// #ifdef VUE3
					dataURL = toRaw(this.canvas).toDataURL()
					// #endif
					// #ifndef VUE3
