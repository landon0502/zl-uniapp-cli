			...
			<template v-if="previewImage">
				<view class="uv-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
					...
					<view 
						...
						<image 
							v-if="item.thumb" 
							:src="item.thumb" :mode="imageMode" 
							class="uv-upload__wrap__preview__image video-thumb" 
							:style="[{
								width: $uv.addUnit(width),
								height: $uv.addUnit(height)
							}]" 
						/>
						<view class="uv-upload__wrap__preview__other__text-box" v-if="item.status !== 'uploading'">
							<uv-icon :color="'#FFFFFF'" size="26" :name="item.isVideo || (item.type && item.type === 'video') ? 'movie' : 'folder'"></uv-icon>
							<text class="uv-upload__wrap__preview__other__text uv-upload__wrap__preview__other__video__text">{{item.isVideo || (item.type && item.type === 'video') ? '视频' : '文件'}}</text>
						</view>
					</view>
...
<style lang="scss" scoped>
	...
	.uv-upload {
		...
		.uv-upload__wrap__preview__other__text-box{
			width: $uv-upload-image-width;
			height: $uv-upload-image-height;
			flex: $uv-upload-other-flex;
			@include flex(column);
		...
		.uv-upload__wrap__preview__other__video__text{
			color: #FFF
		}
	}
</style>