<template>
	<view
		...
		:class="{ 'table--border': border, 'border-none': !(noData && allowAutoShowEmpty) }"
	>
		<!-- #ifdef H5 -->
		<table
			class="uni-table"
		...
		>
			<slot></slot>
			<tr v-if="noData && allowAutoShowEmpty" class="uni-table-loading">
		...
		>
			<slot></slot>
			<view v-if="noData && allowAutoShowEmpty" class="uni-table-loading">
