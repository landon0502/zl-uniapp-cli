...
import { addUnit, addStyle } from '@/uni_modules/uv-ui-tools/libs/function'
import props from './props.js'
import iconUrl from '@/static/iconfont/iconfont.ttf'
import iconfontjson from '@/static/iconfont/iconfont.json'

const fontFamily = 'scmsiconfont'
// #ifdef APP-NVUE
...
export default {
	...
	computed: {
		...
		icon() {
			...
			const code = iconfontjson.glyphs.find((item) => item.name === this.name)?.unicode ?? this.name
			// #ifdef APP-NVUE
			if (!code) {
				return code
					? unescape(`%u${code}`)
...
/* #endif */
/* #ifdef APP-NVUE */
$uv-icon-primary: #255aa6;
$uv-icon-success: #5ac725;
$uv-icon-info: #909399;
$uv-icon-warning: #f9ae3d;
$uv-icon-error: #f56c6c;
/* #endif */
$uv-icon-label-line-height: 1;
/* #ifndef APP-NVUE */
// 非nvue下加载字体
@font-face {
	font-family: 'scmsiconfont';
